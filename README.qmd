...
**igoR** provides access to the Intergovernmental Organizations Database (v3)
from the Correlates of War Project [@pevehouse2020].

The data set (v3) includes information on **`r nrow(igoR::igo_search())`** IGOs
from `r min(igoR::igo_year_format3$year)` to
`r max(igoR::igo_year_format3$year)`, plus membership information.
...
The package also includes a distribution of the State System Membership data
[@correlatesofwarproject2017].
