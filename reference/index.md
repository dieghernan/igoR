# Package index

## Search IGO and state metadata

Find IGO identifiers and resolve state names, abbreviations and codes.

- [`igo_search()`](https://dieghernan.github.io/igoR/reference/igo_search.md)
  : Search for IGOs
- [`igo_search_states()`](https://dieghernan.github.io/igoR/reference/igo_search_states.md)
  : Find Correlates of War state codes, abbreviations and names

## Extract IGO membership data

Extract state-IGO records and derive dyad-year joint memberships.

- [`igo_dyadic()`](https://dieghernan.github.io/igoR/reference/igo_dyadic.md)
  : Extract dyad-year joint IGO membership
- [`igo_members()`](https://dieghernan.github.io/igoR/reference/igo_members.md)
  : Extract IGO membership records by organization
- [`igo_state_membership()`](https://dieghernan.github.io/igoR/reference/igo_state_membership.md)
  : Extract IGO membership records by state

## Recode IGO membership data

Convert numerical membership codes to labeled factors.

- [`igo_recode_igoyear()`](https://dieghernan.github.io/igoR/reference/igo_recode.md)
  [`igo_recode_stateyear()`](https://dieghernan.github.io/igoR/reference/igo_recode.md)
  [`igo_recode_dyadic()`](https://dieghernan.github.io/igoR/reference/igo_recode.md)
  : Recode membership status values

## Included data sets

Data for IGO, country and state system analysis.

- [`igo_year_format3`](https://dieghernan.github.io/igoR/reference/igo_year_format3.md)
  : IGO-year membership data
- [`state_year_format3`](https://dieghernan.github.io/igoR/reference/state_year_format3.md)
  : Country-year IGO membership data
- [`states2024`](https://dieghernan.github.io/igoR/reference/states2024.md)
  : State system membership (v2024)

## About the package

Package overview, authors and links.

- [`igoR`](https://dieghernan.github.io/igoR/reference/igoR-package.md)
  [`igoR-package`](https://dieghernan.github.io/igoR/reference/igoR-package.md)
  : igoR: Access the Intergovernmental Organizations ('IGO') Database
