...
un_1950 <- igo_members("UN", 1950) %>%
  mutate(ISO3_CODE = countrycode(ccode, "cown", "iso3c",
    warn = FALSE
  )) %>%
  left_join(world, .) %>%
  mutate(year = 1950) %>%
...
# Join all
un_all <- bind_rows(un_1950, un_1989, un_2010)
```
...
ggplot(un_all) +
  ...
  scale_fill_manual(values = "#74A9CF", na.value = "#E0E0E0") +
  theme_void() +
  labs(title = "UN Members", caption = gisco_attributions()) +
  theme(plot.caption = element_text(face = "italic"))
```

<div class="figure">
<img src="./UNMaps-1.png" alt="UN Members" width="100%" />
<p class="caption">UN Members</p>
</div>
...
# ISO3 Code
shared$ISO3_CODE <- countrycode(shared$ccode2, "cown", "iso3c",
  warn = FALSE
)
...
<div class="figure">
<img src="./AustShared-1.png" alt="Shared Full Memberships with Australia (2014)" width="100%" />
...
ggplot(countries_sf) +
  ...
  guides(fill = guide_legend(keyheight = 1.5, reverse = TRUE)) +
  labs(
  ...
  theme(
    ...
    plot.caption = element_text(face = "italic", hjust = 0.05),
...
<div class="figure">
<img src="./NAShared-1.png" alt="Shared Full Memberships on North America (1930 - 2010)" width="100%" />
<p class="caption">Shared Full Memberships on North America (1930 - 2010)</p>
</div>
