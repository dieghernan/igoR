...
This vignette is meant to provide **useRs** with an visual, explorable introduction to the capabilites of the `igoR` package.

The analysis would be based on those provided on @doi:10.1177/0022343319881175. For more information on the IGO datasets and additional downloads, see [Intergovernmental Organizations (v3)](https://correlatesofwar.org/data-sets/IGOs).

*Note that the dyadic dataset is not provided in the package, due its size (~500 MB on Stata `.dta` format).*
...
The definition of an Intergovernmental Organization (IGO) on the original dataset is based on the following criteria:
  ...
  1. An IGO must consist of at least three members of the [COW-defined state system](https://correlatesofwar.org/data-sets/cow-country-codes).
...
The datasets begins to code an IGO by identifying the first year in which the
...
This section provides some quick analysis based on the figures of @doi:10.1177/0022343319881175.
...
<details>
  <summary><strong>Custom <code>ggplot2</code> theme</strong>: <code>theme_igoR</code></summary>
...
births <- df %>% mutate(year = sdate) %>%
  group_by(year) %>% summarise(value = n(), .groups = "keep") %>%
  mutate(variable = "IGO Births")

deads <- df %>% mutate(year = deaddate) %>%
  group_by(year) %>% summarise(value = n(), .groups = "keep") %>%
...
Africa <- c( 30, 40, 50, 60, 80, 90, 100, 110, 115, 120, 125, 130, 140,
...
Americas <- c( 310, 320, 330, 340, 720, 760, 815, 875, 880, 890, 900,
...
After we have created a data frame with the regions, we can 
classify the IGOs by region.
...
regionsum <- merge(alligos, regions) %>%
  group_by(year, region) %>% summarise(value = n(), .groups = "keep") %>%
  filter(!is.na(region))
...
asia5_names <-
  cow_country_codes %>% filter(statenme %in% c("India", "China", 
                                               "Pakistan", "Indonesia",
                                               "Bangladesh"))
...
## Analyse number of memberships
numcols <- unlist(lapply(asia5, is.numeric))

# All kinds of Membership
asia5$values <- rowSums(asia5 == 1, na.rm = TRUE) +
  rowSums(asia5 == 2, na.rm = TRUE) +
  rowSums(asia5 == 3, na.rm = TRUE)


asia5 <- asia5[, c("year", "ccode", "statenme", "values")]
