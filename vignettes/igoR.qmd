...
This vignette introduces the main data sets and functions in **igoR** with
visual examples.

The examples build on analyses from @pevehouse2020. For more information about
the IGO data sets and additional downloads, see
...
MB in Stata `.dta` format). The `igo_dyadic()` function derives comparable
joint membership results from the included data.*
...
First, create a custom `ggplot2::theme()` object named `theme_igor`. All
figures in this vignette use this theme.
...
This plot shows the number of IGOs by region. The regional definitions are
based on @pevehouse2020 and the complementary replication data set [@priorep].
