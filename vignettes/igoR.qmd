...
The analysis is based on examples from @pevehouse2020. For more information
about the IGO data sets and additional downloads, see [Intergovernmental
...
Stata `.dta` format). However, `igo_dyadic()` provides similar results.*
...
>
> - An organization is considered terminated when the following words describe
>   the context of the organization: replaced, succeeded, superseded,
>   integrated, merged or dies.
...
First, we create a custom `ggplot2::theme()` named `theme_igor`, which is
applied to all figures:
