---
title: "Get started with igoR"
author: Diego Hernangómez
description: Visual introduction to IGO data and package workflows.
bibliography: REFERENCES.bib
link-citations: true
vignette: >
  %\VignetteIndexEntry{Get started with igoR}
...
This vignette introduces the main data sets and functions in **igoR** through
visual examples.

The examples build on analyses from @pevehouse2020. For more information about
the IGO data sets and additional downloads, see [Intergovernmental Organizations
(version 3)](https://correlatesofwar.org/data-sets/IGOs/).

*The original dyad-year data set is not included because of its size (about 500
MB in Stata `.dta` format). The `igo_dyadic()` function derives comparable joint
membership results from the included data.*
...
> ### What is an IGO?
>
> The original data set defines an intergovernmental organization (IGO) using
> the following criteria:
>
> 1.  An IGO must consist of at least three members of the [Correlates of War
>     state system](https://correlatesofwar.org/data-sets/cow-country-codes/).
...
> functions. In some cases, individual members are recorded by year of accession
> or signature.
>
...
>
> - An organization is considered terminated when its description includes one
>   of the following terms: `replaced`, `succeeded`, `superseded`, `integrated`,
>   `merged` or `dies`.

## Analysis

This section reproduces selected analyses based on figures in @pevehouse2020.
...
First, create a custom `ggplot2::theme()` object named `theme_igor`. All figures
in this vignette use this theme.
...
The following code counts IGOs and states in the Correlates of War (COW) system
by year. The data cover 1816 to 2014.
...
This plot shows the number of IGOs by region. The region definitions are based
on @pevehouse2020 and the complementary replication data set [@priorep].
...
The next step joins the region classifications to the IGO-year data and counts
IGOs by region.
...
This plot shows the number of full IGO memberships for five states in Asia:
...
The final plot counts full joint memberships in dyad-year data for Spain and
