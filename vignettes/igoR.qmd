...
*The dyad-year data set is not included in the package because of its size
(\~500 MB in Stata `.dta` format), but `igo_dyadic()` provides comparable
joint membership results.*
...
The following code counts IGOs and COW states included in the package. The
...
The final plot counts dyad-year full joint memberships between Spain and four
selected states.
