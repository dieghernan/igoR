# Testing messages 
    ...
    Message
      State value not found: 'Error'.
    Condition
      Warning in `igo_state_membership()`:
      State value(s) 'Error' were not found in the database.
...
---
    ...
    Message
      State 'modena' is available only between 1842 and 1860.
    Condition
      Warning in `igo_state_membership()`:
...
---
    ...
    Condition
      Warning in `igo_state_membership()`:
      Invalid `status` value(s): 'Nope', 'IGO Not In Existence'. Valid values are 'No Membership', 'Full Membership', 'Associate Membership', 'Observer', 'Missing data', 'State Not System Member'.
...
---
    ...
    Message
      No IGOs were found for state 'spain' with the arguments provided.
    Condition
      Warning in `igo_state_membership()`:
      No states found with the required arguments.
...
# Extract several cntries
    ...
    Code
      sev <- igo_state_membership(c("UnitEd KingDom", "SPAIN", "aga haha", "1298",
        "WGeRMANy"))
    Message
      State value not found: 'aga haha'.
      State value not found: '1298'.
...
# Cleanup
    ...
    Message
      State value not found: 'invented'.
